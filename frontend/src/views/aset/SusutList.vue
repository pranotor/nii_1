   ...
   <vs-row vs-justify="center">
    <vs-col type="flex" vs-justify="center" vs-align="center" vs-lg="12" vs-xs="12">
      <vs-card>  
        ...
        <h3 class="card-title mb-0">Penyusutan Aktiva</h3>
        <div class="ml-auto">
          <vs-button @click="inputForm"  color="success" type="filled" class="mt-4 mt-md-0">
            <i class="mdi mdi-border-color mr-1"></i> Proses Penyusutan
          </vs-button>
        </div> 
        </div>
        <hr class="custom-hr" />  
        ...
        <JqxMenu ref="myMenu" @itemclick="myMenuOnItemClick($event)"
            ...
            <ul>
                <li>Lihat Hasil Susut</li>
                <li>Batal Proses Susut</li>
            </ul>
        </JqxMenu>
      </vs-card>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        width: "100%",
        columns: [
          { text: 'Tgl Proses', datafield: 'tgl_trans', filtertype: 'date', width: '20%', cellsformat: 'dd-MM-yyyy' },
          { text: 'Referensi', datafield: 'referensi', width: '20%' },
          { text: 'Tahun', datafield: 'tahun', width: '30%' },
          { text: 'Bulan', datafield: 'bulan', width: '30%' }
        ],
        selectrow: 0,
        popupPdf: false,
        pdfsrc: ''
    ...
    methods: {
        ...
        inputForm: function(){
            this.$router.push({name : 'susutproses'});
        },
        ...
        myMenuOnItemClick: function (event) {
            ...
            if (args.innerHTML == 'Lihat Hasil Susut') {
                ...
                console.log(data)
                if(confirm("Yakin membatalkan proses penyusutan?")){
                    this.$vs.loading({'type':'radius'})
                    this.$http.post('/api/susutbatal',{
                        tahun : data.tahun,
                        bulan : data.bulan,
                        ref : data.referensi,
                    }).then(resp => {
                        ...
                        this.$vs.notify({title:'BERHASIL..',text:'Penyusutan berhasil dibatalkan..',color:'success'})
                    }).catch(error => {
                        this.$vs.loading.close()
                        this.$vs.notify({title:'GAGAL..',text:'Gagal Proses..',color:'danger'})
                    })
                }
            }
        }
    ...
    beforeCreate: function () {
      this.source = {
        ...
        datatype: 'json',
        id: 'referensi',
        url: this.$urlbackend+'/api/susutlist',
        type: 'post',
