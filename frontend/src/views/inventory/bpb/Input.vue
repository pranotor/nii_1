        ...
        <vs-row vs-justify="center">
            ...
            <vs-card>
                <div class="d-md-flex align-items-center pb-2">
                    <h3 class="card-title mb-0">Good Receive (BPB) {{this.$route.params.ref}}</h3>
                    <div class="ml-auto">
                ...
                <h5 class="card-title mb-0" v-if="!this.$route.params.ref">Pilih PO</h5>
                <JqxDropDownButton ref="myDropDownButton" :width="150" :height="25" v-if="!this.$route.params.ref">
                ...
                <vs-col vs-lg="6" vs-xs="12" vs-sm="4">  
                    <vs-input label="NOMOR SJ" class="w-100 mt-4" v-model="formdata.no_sj"/>  
                </vs-col>
                ...
                <div>            
                    <vs-table  :data="formdata.datatrans" class="text-nowrap">
                        ...
                        <template slot="thead">
                            ...
                            <vs-th>Jumlah Diterima</vs-th>
                            <vs-th></vs-th>
                        </template>
                        <template slot-scope="{data}">
                            <vs-tr class="border-top" :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                            <vs-td>{{ data[indextr].kode_barang}}&nbsp;&nbsp;&nbsp;{{ data[indextr].itembarang.kode_barang}}</vs-td>
                            <vs-td vs-align="right">{{ data[indextr].nama2 }}</vs-td>
                            <vs-td vs-align="right">{{ data[indextr].itembarang.satuan }}</vs-td>
...
<script>
    ...
    data: function () {
      return {
        ...
        width: "650",
        columns: [
            { text: 'PO NO', datafield: 'po_no', width: '100', },
            { text: 'Tgl', datafield: 'po_tgl', filtertype: 'range', width: '100', cellsformat: 'dd-MM-yyyy' },
    ...
    methods: {
        ...
        myGridOnRowSelect: function (event) {
            if(this.$route.params.ref != null){
                ...
                let dropDownContent = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['po_no'] + '</div>';
                this.$refs.myDropDownButton.setContent(dropDownContent);
                this.formdata.po_no = row['po_no']
                this.getPO(row['po_no']).then().catch(error=>{
                    this.$vs.notify({title:'Error',text:'Gagal inisiasi PO, silahkan coba lagi...',color:'danger'})
                })
            }
        ...
        backpage: function(){
            this.$router.push({name : 'bpblist'});
        },
        async getPO(referensi){
            try{
                let response = await this.$http.get("/api/getPO/"+referensi)
        ...
        getAllData: function(){
            ...
            if(this.$route.params.ref != null){
                ...
                this.$http.get("/api/bpb/"+this.$route.params.ref).then(response => {
                    console.log(response.data)
                    this.formdata.datatrans = response.data;
                    ...
                    for(const key of keys){
                        ...
                        subkeys.forEach(item => {
                            ...
                            if(item == 'debet' || item == 'kredit'){
                                this.formdata.datatrans[key][item] =  new Intl.NumberFormat('us-US',{notation: 'standard',}).format(response.data[key][item])
                            }
                            if(item == 'id')
                            ...
                            if(item == 'po_no'){
                                ...
                                /* let dropDownContent = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['po_no'] + '</div>';
                                this.$refs.myDropDownButton.setContent(dropDownContent); */
                                console.log(response.data[key][item]);
                                this.getPO(response.data[key][item]).then().catch(error=>{
                                    this.$vs.notify({title:'Error',text:'Gagal inisiasi PO, silahkan coba lagi...',color:'danger'})
                                })
                            }
                            if(item == 'datatrans'){
    ...
    computed: {
        ...
        sumBayar() {
            ...
            let str_ret = new Intl.NumberFormat('us-US',{notation: 'standard',}).format(sum)
