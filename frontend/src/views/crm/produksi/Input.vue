        ...
        <vs-row vs-justify="center">
            ...
            <vs-card>
                <div class="d-md-flex align-items-center pb-2">
                    <h3 class="card-title mb-0">Proses Produksi {{this.$route.params.ref}}</h3>
                    <div class="ml-auto">
                ...
                <h5 class="card-title mb-0">Pilih WO</h5>
                ...
                <div>
                    ...
                    <vs-row>
                        <vs-col vs-lg="7" vs-xs="12" vs-sm="4">  
                            <vs-select class="w-100 mt-1" label="Kode Barang - Nama Barang" ref="barang" v-model="formdata.barang_hasil" 
                            ...
                            <vs-select-item
                                ...
                                :value="item.id"
                                :text="item.kategori"
                                v-for="(item,index) in optWODet"
                            />
                            </vs-select>
                        </vs-col>
                        
                        <vs-col vs-lg="1" vs-xs="12" vs-sm="4">  
                            <vs-input label="Satuan" class="w-100 mt-1" v-model="satuan_hasil" disabled/> 
                        </vs-col>
                        <vs-col vs-lg="3" vs-xs="12" vs-sm="4">  
                            <vs-input label="JUMLAH" class="w-100 mt-1" v-model="formdata.qty_hasil" v-currency="options"/> 
                        </vs-col>
                    </vs-row>
                ...
                <div> 
                    ...
                    <vs-row>
                        ...
                        <vs-col vs-lg="3" vs-xs="12" vs-sm="4">  
                            <vs-input label="JUMLAH" class="w-100 mt-1" v-model="qty" v-currency="options"/> 
                        </vs-col>
...
<script>
    ...
    data: function () {
      return {
        ...
        columns: [
            ...
            { text: 'Tgl WO', datafield: 'wo_tgl', filtertype: 'date', width: '100', cellsformat: 'dd-MM-yyyy' },
            { text: 'Tgl SO', datafield: 'so_tgl', filtertype: 'date', width: '100', cellsformat: 'dd-MM-yyyy' },
    ...
    beforeCreate: function () {
      this.source = {
        datafields: [
          ...
          { name: 'so_tgl', map: 'quotation>so_tgl', type:'date'},
    ...
    methods: {
        ...
        myGridOnRowSelect: function (event) {
            ...
            this.formdata.barang_hasil = '';
            this.optWODet = [];
            let args = event.args;
            let row = this.$refs.myGrid.getrowdata(args.rowindex);
            //console.log(row);
            let dropDownContent = '<div style="position: relative; margin-left: 3px; margin-top: 5px;">' + row['wo_no'] + '</div>';
            this.$refs.myDropDownButton.setContent(dropDownContent);
            this.formdata.wo_id = row['id']
            ///* this.formdata.qt_id = row['id']
            this.getWO(row['id']).then().catch(error=>{
        ...
        removeTrans: function(id) {
            ...
            let qty = this.formdata.datatransprod[index].qty
            ...
            stok_barang = stok_barang + Number(qty)
            obj_barang.stock.sisa = stok_barang
            //this.formdata.datahapus.push(this.formdata.datatransprod[index])
        ...
        addTrans: function(){
            this.errorQty = false
            if(this.barang!='' && this.qty!=''){
                //cek permintaan terhadap stock
                //let minta = Number(this.qty)
                let minta = this.numberValue(this.qty)
                ...
                if(sisa - minta >=0){
                    ...
                    console.log(obj_barang)
                    let stok_barang = Number(obj_barang.stock.sisa)
                    stok_barang = stok_barang - Number(this.qty)
                    obj_barang.stock.sisa = stok_barang
        ...
        checkForm:function(e) {
            ...
            if(this.formdata.barang_hasil == ''){
                this.errors.push("Barang hasil produksi belum terisi...")
            }
