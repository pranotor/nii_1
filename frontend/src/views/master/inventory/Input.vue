   ...
   <vs-row vs-justify="center">
    <vs-col vs-lg="12" vs-xs="12" vs-sm="12">
      ...
      <vs-card>
        ...
        <h3 class="card-title mb-0">Inventory {{this.$route.params.ref}}</h3>
        <div class="ml-auto">
        ...
        <div>
          <vs-row>
            <vs-col vs-lg="8" vs-xs="12" vs-sm="3">  
                <vs-select class="w-100 mt-4" label="KODE PERKIRAAN" ref="perkiraan" v-model="formdata.kode_perk">
                <vs-select-item
                    :key="index"
                    :value="item.kode_perk"
                    :text="parseNama(item.nama)"
                    v-for="(item,index) in optperkiraanInv"
                />
                </vs-select>
            </vs-col>
            <vs-col vs-lg="2" vs-xs="12" vs-sm="3">  
                <vs-input label="KODE BARANG" class="w-100 mt-4" ref="transval" v-model="formdata.kode_barang" /> 
            </vs-col>
            <vs-col vs-lg="2" vs-xs="12" vs-sm="3">  
                <vs-input label="SATUAN" class="w-100 mt-4" ref="transval" v-model="formdata.satuan" /> 
            </vs-col>
          </vs-row>
          <vs-row>
            <vs-col vs-lg="12" vs-xs="12" vs-sm="3">  
                <vs-input label="URAIAN" class="w-100 mt-4" ref="transval" v-model="formdata.uraian" /> 
            </vs-col>
          ...
          <vs-row>
            <vs-col vs-lg="2" vs-xs="12" vs-sm="3">
                ...
                <vs-switch v-model="formdata.use_sn">
    ...
    methods:{
        ...
        getAllData: function(){
            this.$http.get('/api/perkiraan').then(response => {
                ...
                this.optperkiraanInv = this.optperkiraan.filter(function(kode){
                    if(kode.kode_perk.substring(0,2) == '15')
                        return true;
                })
            }).catch(error=>{
                //console.log(error);
        ...
        checkForm:function(e) {
            ...
            if(this.formdata.kode_barang == ''){
                this.errors.push("Kode Barang belum terisi...")
            }
