  ...
  export default {
    ...
    data: function () {
      return {
        ...
        width: "100%",
        columns: [
          { text: 'User ID', datafield: 'email', width: '20%', },
          { text: 'Nama', datafield: 'name', width: '20%', },
          { text: 'Role', filtertype: 'list', filteritems: this.items, datafield: 'role', width: '60%'}
        ],
        selectrow: 0,
        routeEdit: 'useredit'
      }
        ...
        myMenuOnItemClick: function (event) {
            ...
            if (args.innerHTML == 'Edit') {
                ...
                if(confirm("Yakin menghapus data?")){
                    this.$http.delete('/api/userapp/' + data.id).then(resp => {
                        console.log(resp)
    ...
    beforeCreate: function () {
      this.source = {
        datafields: [
          ...
          { name: 'role', map: 'userrole>role_name'},
        ],
        datatype: 'json',
        id: 'kode',
        url: this.$urlbackend+'/api/userapp',
        type: 'get',
