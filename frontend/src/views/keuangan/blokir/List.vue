      ...
      <vs-card>  
        ...
        <div class="ml-auto">
          <vs-button @click="inputForm"  color="success" type="filled" class="mt-4 mt-md-0">
            <i class="mdi mdi-border-color mr-1"></i> Tambah Data
          </vs-button>
        </div> 
        </div>
        <hr class="custom-hr" />  
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        columns: [
          ...
          { text: 'Nick', datafield: 'nick', width: '15%', },
          { text: 'Alamat', datafield: 'alamat', width: '30%'},
          { text: 'Kota', datafield: 'kota', width: '10%'},
          { text: 'Status', datafield: 'blokstatus', width: '20%'},
        ],
        selectrow: 0,
    ...
    methods: {
        ...
        inputForm: function(){
            this.$router.push({name : 'blokirinput'});
        },
        acceptAlert(){
            let formdata = {}
            formdata.cust_id = this.cust_id
            formdata.blokstat = 2;
            this.$http.post('/api/customerblokir',{
                ...
                if(resp.data.message=='need approval'){
                    this.$vs.dialog({
                        color:this.colorAlert,
                        title: `Membutuhkan Persetujuan`,
                        text: 'Jatuh tempo tagihan lebih dari 90 hari, membutuhkan persetujuan buka blokir',
                        acceptText :'Ya',
                    })
                }
                else
                    this.$vs.notify({title:'BERHASIL..',text:'Buka blokir berhasil diproses..',color:'primary'})
                this.$refs.myGrid.updatebounddata();
                //this.$vs.notify({title:'BERHASIL..',text:'Pengajuan buka blokir sedang diproses..',color:'primary'})
            }).catch(error => {
                this.$vs.notify({title:'GAGAL PROSES..',text:'Buka blokir gagal diproses..',color:'danger'})
            })
        ...
        myMenuOnItemClick: function (event) {
            ...
            else if(args.innerHTML == 'Buka Blokir'){
                this.editrow = rowindex;
                let dataRecord = this.$refs.myGrid.getrowdata(this.editrow)
                this.cust_id = dataRecord.id
                this.$vs.dialog({
                    color:this.colorAlert,
                    title: `Konfirmasi Buka Blokir`,
                    text: 'Yakin akan membuka blokir ?',
                    acceptText :'Ya',
    ...
    beforeCreate: function () {
      this.source = {
        datafields: [
          ...
          { name: 'blokstatus', map: 'status>status_name'},
        ],
        datatype: 'json',
        id: 'id',
        url: this.$urlbackend+'/api/customerblokir',
        type: 'get',
