            ...
            <vs-table  :data="formdata.invoices" class="text-nowrap">
                ...
                <template slot-scope="{data}">
                    ...
                    <vs-td vs-align="right">
                        <vs-input v-model="data[indextr].total" v-currency="options" disabled style="width:150px"/>
                    </vs-td>
                    <vs-td vs-align="right">
   ...
   export default {
     ...
     methods:{
         ...
         onRowselect: async function (event) {
            ...
            await rows.forEach(item => {
                ...
                self.formdata.inv_id.push(dataRecord.id)
            ...
            let formatInv = new Intl.NumberFormat('us-US',{style: 'decimal',maximumFractionDigits: 2}).format(sumInvoice)
            this.formdata.trans_jumlah = formatInv;
         },
         ...
         getAllData: function(){
             ...
             if(this.$route.params.ref != null){
                 ...
                 this.$http.get("/api/banktrans/"+this.$route.params.ref).then(response => {
                     ...
                     for(const key of keys){
                         ...
                         if(key == 'trans_tgl'){
                                 ...
                                 this.formdata.trans_tgl = tgltrans
                         }      
                         if(key == 'trans_no'){
                                this.formdata.prevReferensi = response.data[key]
                                this.formdata.referensi = response.data[key]
                         }
                         if(key == 'bayar'){
                                this.formdata.invoices = response.data[key]
                         }
                     }
                 }).catch(error=>{
                     console.log(error)
     ...
     beforeCreate: function () {
      this.source = {
        ...
        datatype: 'json',
        id: 'referensi',
        url: this.$urlbackend+'/api/piutang',
        type: 'get',
