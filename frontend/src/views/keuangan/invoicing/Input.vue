    ...
    <vs-col vs-lg="12" vs-xs="12" vs-sm="12">
      <vs-card>
        ...
        <vs-alert v-if="errors.length" class="mb-3 mt-2"  color="danger">
          ...
          <vs-row>
            <vs-col vs-lg="4" vs-xs="12" vs-sm="4">  
                <vs-select class="w-100 mt-4" ref="rekanan" label="Nomor FP" v-model="formdata.no_fp">
                <vs-select-item
                    :key="index"
                    :value="item.no_seri"
                    :text="item.no_seri"
                    v-for="(item,index) in optfp"
                />
                </vs-select>
            </vs-col>
          </vs-row>
...
<script>
  export default { 
    ...
    methods:{
        ...
        getAllData: function(){
            this.$http.get('/api/nomorfp_avail').then(response => {
                //console.log(response.data);
                this.optfp = response.data
            }).catch(error=>{
                //console.log(error);
            });
        ...
        checkForm:function(e) {
            this.errors = [];
            if(this.formdata.no_fp == '' || this.formdata.no_fp == '-'){
